...
class Hop < ActiveRecord::Base
    ...
    def ingr_type=(new_hop_type)
       self.hop_type = new_hop_type 
    end

    has_many :hops_inventory_log_entries
    def log_entries
        return self.hops_inventory_log_entries
    end
    ...
    named_scope :list, :include => :hop_type, :order => "hop_types.name"
    named_scope :type_minutes, :include => :hop_type, :order => "hop_types.name, minutes"
    ...
    def weight( for_aa = nil )
        for_aa = aa if for_aa.nil?
        return (dry_hop_amount_l ? dry_hop_amount_l : 0.0) * recipe.volume if no_utilisation?
        
        wgrams = (recipe.volume * ibu_l) / (utilisation(recipe.total_points, adjusted_mins, recipe.hop_utilisation_method) * (for_aa/100) * 1000.0)
        return wgrams
    end
    ...
    def adjust( factor, old_og, new_og )
    ...
    def total_recipe_hops_weight_new_aa( new_aa )
        return if ( new_aa.nil? || new_aa <= 0.0 )
        
        total_weight = 0.0

        recipe.hops.scoped( :conditions => ["hop_type_id = ?", hop_type] ).each do |hop|
            next unless hop
            # Only do consider aa changes for additions latter than 15 minutes.
            if( hop.minutes >= 15.0 ) then 
                total_weight = total_weight + hop.weight(new_aa)
            else
                total_weight = total_weight + hop.weight
            end 
        end
    ...
    def no_utilisation?
        logger.debug "++ hop.no_utilisation: hop_type: #{hop_type}"
