...
module RecipesHelper
    ...
    def update_details_and_fermentables( recipe )
        ...
        render(:update) { |page|
            ...
            page << "if( jQuery('#hops_content_div').length  > 0 ) {"
            page.replace_html 'hops_items_div', :partial => 'shared/recipe_edit_hops_items', :object => recipe
            page << "}"
    ...
    def update_details_and_fermentables_and_kits( recipe )
        ...
        render(:update) { |page|
            ...
            # Check to see if the tab has been loaded before updating.  Is a bit wasteful sending data if it is not loaded
            # but a better user experience then triggering a second ajax call to do the specific tabs update.

            page << "if( jQuery('#hops_content_div').length  > 0 ) {"
            page.replace_html 'hops_items_div', :partial => 'shared/recipe_edit_hops_items', :object => recipe
            page << "}"

            page << "if( jQuery('#kits_content_div').length  > 0 ) {"
    ...
    def ajax_weight_editor( fermentable, action=:update, controller=:fermentables, render="details_and_fermentable", attribute="weight", prefix="fw" )
        ajax_edit_field2( fermentable, attribute,
                         { :action => action, :controller => controller, :id => fermentable.id, :render => render },
                         BrewingUnits::values_array_for_display( current_user.units.fermentable, fermentable.send(attribute), 2 ),
                         nil, nil, "#{prefix}_#{fermentable.id}")

    end

    def ajax_editor( item, action, controller, render, attribute )
        ajax_edit_field2( item, attribute,
                         { :action => action, :controller => controller, :id => item.id, :render => render }, nil,
                         nil, nil, "fw_#{attribute}_#{item.id}")
    end
    ...
    def edit_inv_usage(inv_item, recipe, ingr, item_holder)

        # Find or create the amount.
        inv_log_type = Object.const_get( inv_item.class.name + "LogEntry" )
        inv_recipe_log_type_name = (inv_item.class.name + "LogEntries" ).underscore
        inv_recipe_log = Object.const_get(inv_item.class.name + "LogEntry")
        inv_type_name = inv_item.class.name.underscore
        ingr_name = ingr.class.name.underscore

        item = inv_recipe_log.find(:all, 
            :conditions => { "#{inv_type_name}_id" => inv_item, "#{ingr_name}_id" => ingr } ).first

        # If we dont find it create a new one,
        if item.nil?
            item = inv_log_type.create do |inv|
                inv.inventory = inv_item
                inv.recipe = recipe
                inv.amount = 0.0
                inv.ingr = ingr
        ...
        item.save
        item_holder << item
        # Pass to the ajax editor, need to change for yeasts and kits.
        ajax_weight_editor( item, :update_invusage, :brew_entries, "", :amount )
    end
    ...
    def add_inventory_link(item_type, inventory_class)

        invent_type = inventory_class.name

        invent_class_txt = invent_type.gsub(/Inventory/, '')

        link_to_remote( "Add",
                       :loading => "Hobo.showSpinner('Adding #{invent_class_txt}');",
                       :complete => "Hobo.hideSpinner();",
                       :url => { :controller => :inventories, :action => :add_item, :item_id => item_type.id, :item_type => item_type.class.name,  :invent_type => invent_type },
                       :html => { :class => "del_button" } )


    end

    def del_inventory_link(item)

        type = item.class.name

        class_txt = type.gsub(/Inventory/, '')

        link_to_remote( "Del",
                       :loading => "Hobo.showSpinner('Delete #{class_txt}');",
                       :complete => "Hobo.hideSpinner();",
                       :url => { :controller => :inventories, :action => :remove_item, :id => item.id, :type => type },
                       :html => { :class => "del_button" } )
    end
